...
extension MJRefreshNormalHeader {
    static func customHeader(refreshingTarget target: Any?, refreshingAction action: Selector?) -> MJRefreshNormalHeader {
        ...
        header.setTitle("MJRefreshHeaderIdleText".localized(), for: .idle)           // 普通状态
        header.setTitle("MJRefreshHeaderPullingText".localized(), for: .pulling)        // 松开刷新状态
        header.setTitle("MJRefreshHeaderRefreshingText".localized(), for: .refreshing)  // 正在刷新状态
        header.lastUpdatedTimeText = { (lastUpdatedTime) -> String in
            guard let time = lastUpdatedTime else { return "MJRefreshHeaderNoneLastDateText".localized() } // 没有更新时间时显示
            let formatter = DateFormatter()
            formatter.dateFormat = "yyyy-MM-dd HH:mm"
            return "MJRefreshHeaderLastTimeText".localized() + formatter.string(from: time)
        }
